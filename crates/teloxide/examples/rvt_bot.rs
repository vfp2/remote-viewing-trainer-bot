...
enum Command {
    ...
    Help,
    #[command(description = "Start a remote viewing session")]
    Start,
}
...
/// or not, then match the command. If the command is `/start` it starts a RV session.
    ...
    if let Some(text) = msg.text() {
        match BotCommands::parse(text, me.username()) {
            ...
            Ok(Command::Start) => {
                ...
                bot.send_message(msg.chat.id, "Step 1/6: Preparation\nAre you ready?").reply_markup(keyboard).await?;
            }
